...
const api = {
  ...
  user: {
    ...
    updateProfile:   (body) => api.put('/user/update-profile',  body),
    changePassword:  (body) => api.post('/user/change-password', body),
    getPrefs:        ()     => api.get('/user/preferences'),
    savePrefs:       (body) => api.put('/user/preferences',     body),
    history:         ()     => api.get('/user/history'),
    deleteHistory:   ()     => api.delete('/user/history'),
  },
